...
def get_appointments_for_case(self, meeting):
    return [a for a in self.schedule.appointments if a.meeting == meeting]

def add_appointments_for_case(self, meeting, judge, room, day, start_timeslot):
    for t in range(meeting.meeting_duration // self.schedule.granularity):
        self.schedule.appointments.append(Appointment(meeting, judge, room, day, start_timeslot + t))
...
def initialize_quality_test(self):
    ...
    for judge_idx, room_idx, meeting_indeces in assignments:
        judge = judges[judge_idx]
        room = rooms[room_idx]
        day = 1 
        current_time = 1  
        
        for meeting_idx in meeting_indeces:
            meeting = meetings[meeting_idx]
            slots_needed = meeting.meeting_duration // self.schedule.granularity
...
class TestRoomStabilityRule(unittest.TestCase):
    ...
    def setUp(self):
        ...
        case5 = Case(5)
        case6 = Case(6)
        self.meeting1 = Meeting(1, 30, 0, None, None, case1)
        self.meeting2 = Meeting(2, 30, 0, None, None, case2)    
        self.meeting3 = Meeting(3, 30, 0, None, None, case3)
...
# FULL score calculations
    ...
    def test_no_room_changes(self):
        add_appointments_for_case(self, self.meeting1, self.judge1, self.room1, 1, 1)
        add_appointments_for_case(self, self.meeting2, self.judge1, self.room1, 1, 8)
    ...
    def test_two_room_changes(self):
        add_appointments_for_case(self, self.meeting1, self.judge1, self.room1, 1, 1)
        add_appointments_for_case(self, self.meeting2, self.judge1, self.room2, 1, 8) # changes both ways
        add_appointments_for_case(self, self.meeting3, self.judge1, self.room1, 1, 15)
...
# DELTA score calculations
    ...
    def simple_multiple_moves_scenario(self):
        add_appointments_for_case(self, self.meeting1, self.judge1, self.room1, 1, 1)
        add_appointments_for_case(self, self.meeting2, self.judge1, self.room2, 1, 8)
        add_appointments_for_case(self, self.meeting3, self.judge2, self.room1, 1, 1)
        add_appointments_for_case(self, self.meeting4, self.judge2, self.room2, 1, 8)
        
        # assert full score is 2
        violations = room_stability_per_day_full(self.schedule)
        self.assertEqual(violations, 2)
        
        # move meeting 1 to judge 2 timslot 14 => assert full score is 2 and delta score is 0)
        appointments = get_appointments_for_case(self, self.meeting1)
        move = Move(meeting_id = 1, appointments = appointments, old_judge= self.judge1, new_judge= self.judge2, old_room= self.room1, new_room= self.room1, old_day= 1, new_day= 1, old_start_timeslot= 1, new_start_timeslot= 14)
        apply_move_to_schedule(self.schedule, move)

        violations = room_stability_per_day_full(self.schedule)
        self.assertEqual(violations, 2)
        
        # now move meeting 4 to judge 1 timeslot 1 => assert full score is 0 and delta score is -2
        appointments = get_appointments_for_case(self, self.meeting4)
        move = Move(meeting_id = 4, appointments = appointments, old_judge= self.judge2, new_judge= self.judge1, old_room= self.room2, new_room=None, old_day= 1, new_day=None, old_start_timeslot= 8, new_start_timeslot= None)
    ...
    def test_day_change_only(self):
        ...
        add_appointments_for_case(self, self.meeting1, self.judge1, self.room2, 1, 8)
        add_appointments_for_case(self, self.meeting2, self.judge1, self.room1, 1, 15)
        add_appointments_for_case(self, self.meeting3, self.judge1, self.room1, 2, 1)
        add_appointments_for_case(self, self.meeting4, self.judge1, self.room2, 2, 8)
        
        violations = room_stability_per_day_full(self.schedule)
        self.assertEqual(violations, 2, "Initial violations should be 2")
        
        # Move meeting 2 to day 2 - this reduces violations by 1 on day 1 but increases them by 1 on day 2
        appointments = get_appointments_for_case(self, self.meeting2)
        move = Move(meeting_id=2, appointments=appointments, 
    ...
    def test_judge_change_only(self):
        ...
        # Judge1 has meeting 1 & 2 with a room change
        # Judge2 has meeting 3 & 4 with a room change
        # Total violations = 2
        add_appointments_for_case(self, self.meeting1, self.judge1, self.room1, 1, 15)
        add_appointments_for_case(self, self.meeting2, self.judge1, self.room2, 1, 8)
        add_appointments_for_case(self, self.meeting3, self.judge2, self.room1, 1, 1)
        add_appointments_for_case(self, self.meeting4, self.judge2, self.room2, 1, 8)
        ...
        # Move meeting1 to judge2 
        appointments = get_appointments_for_case(self, self.meeting1)
        move = Move(meeting_id=1, appointments=appointments, 
    ...
    def test_room_change_only(self):
        ...
        # Move meeting2 to room1 - only changing room
        appointments = get_appointments_for_case(self, self.meeting2)
        move = Move(meeting_id=2, appointments=appointments, 
    ...
    def test_timeslot_change_only(self):
        ...
        add_appointments_for_case(self, self.meeting1, self.judge1, self.room1, 1, 1)
        add_appointments_for_case(self, self.meeting2, self.judge1, self.room2, 1, 8)
        add_appointments_for_case(self, self.meeting2, self.judge1, self.room2, 1, 22)
        
        violations = room_stability_per_day_full(self.schedule)
        self.assertEqual(violations, 1, "Initial violations should be 1")
        
        # Move meeting1 to timeslot between meeting2 and meeting3 - only changing timeslot
        appointments = get_appointments_for_case(self, self.meeting1)
        move = Move(meeting_id=1, appointments=appointments, 
                    old_judge=self.judge1, new_judge=None,  # Not changing judge
    ...
    def test_multiple_parameter_changes(self):
        ...
        add_appointments_for_case(self, self.meeting1, self.judge1, self.room1, day=1, start_timeslot=1)
        add_appointments_for_case(self, self.meeting2, self.judge1, self.room1, day=1, start_timeslot=8)
        add_appointments_for_case(self, self.meeting3, self.judge2, self.room2, day=1, start_timeslot=1)
        add_appointments_for_case(self, self.meeting4, self.judge2, self.room2, day=1, start_timeslot=8)
        add_appointments_for_case(self, self.meeting5, self.judge2, self.room3, day=2, start_timeslot=1)
        add_appointments_for_case(self, self.meeting6, self.judge2, self.room3, day=2, start_timeslot=15)
        ...
        # Move meeting2 to judge2, room1, day 2 - changing judge, room, day, and timeslot
        appointments = get_appointments_for_case(self, self.meeting1)
        move = Move(meeting_id=1, appointments=appointments, 
                    old_judge=self.judge1, new_judge=self.judge2, 
